...
clear
import delimited "Data_Publications/NPSM_Maintable1.csv", clear 
...
reghdfe lnvisits t_above90 t_80 t_60 t_50 t_40 t_30 t_less30 lightrain moderaterain heavyrain lndisposable lnfee lngas unemploy lnstatepop per_65 nine_11 designation instalaunch uploadshigh uploadsmid uploadslow if year>=1993 & year<2020, absorb(i.park#i.month year) vce(cluster park yearmonth)  
...
coefplot (D*  , pstyle(p13)), keep(t_above90 t_80 t_60 t_50 t_40  t_30 t_less30 lightrain moderaterain  heavyrain) rename(t_above90=90F t_80=80F-90F t_60=60F-70F t_50=50F-60F t_40=40F-50F t_30=30F-40F  t_less30=30F<  lightrain=Light moderaterain=Moderate  heavyrain=Heavy) groups(90F 80F-90F 60F-70F 50F-60F 40F-50F 30F-40F 30F< = "{bf:Temperature}" Light Moderate Heavy = "{bf:Rain}")  coeflabels() grid(none) xline(0, lpattern(solid) lcolor(red)) msymbol(d) cismooth levels(95) ciopts(lwidth(3 ..) lcolor(*.2 *.4 *.6 *.8 *1)) mcolor() xlab(, nogrid) xtitle("Log(Visitation)")  xscale(range(-0.04 .1))
graph export "/Users/ashleylowe/NPS/fig3d.png", as(png) replace

coefplot (D* , asequation() \ , pstyle(p13)), keep(lndisposable lnfee lnstatepop per_65 lndisposable unemploy nine_11 designation) rename(lndisposable=Ln(RDI) lngas=Ln(Gas) lnfee=Ln(Fee)  lnstatepop=Ln(StatePop) per_65=Per.65+  unemploy=Unemploy nine_11=Post9/11 designation=Park.Desig) groups(Ln(RDI) Ln(Gas) Ln(Fee) Ln(StatePop) Per.65+ Unemploy  Post9/11 Park.Desig= "{bf:Trends & Demographics}" )  coeflabels() grid(none) xline(0, lpattern(solid) lcolor(red)) msymbol(d) cismooth levels(95) ciopts(lwidth(3 ..) lcolor(*.2 *.4 *.6 )) mcolor() xlab(, nogrid)  xscale(range(-2 6))
graph export "/Users/ashleylowe/NPS/fig3dd.png", as(png) replace


coefplot (D, asequation("{bf:All Parks}") \ E, asequation("{bf:PUDs by Viral Quality}") \ , pstyle(p1)), keep( uploads uploadshigh uploadsmid uploadslow ) rename(uploads=PUDs uploadshigh=High uploadsmid=Moderate uploadslow=Low) coeflabels() grid(none) xline(0, lpattern(solid) lcolor(red)) msymbol(d) cismooth levels(95) ciopts(lwidth(3 ..) lcolor(*.4 *.6 )) mcolor() xlab(, nogrid) xtitle("Log(Visitation)")
